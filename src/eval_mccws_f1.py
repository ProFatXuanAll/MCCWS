...
def main(argv: List[str]) -> None:
  ...
  train_cfg = json.load(open(
    os.path.join(src.vars.EXP_PATH, args.exp_name, 'train_cfg.json'),
    'r',
    encoding='utf-8',
  ))
  ...
  score_script_path = os.path.join(src.vars.PROJECT_ROOT, 'scr', 'score')
  for ckpt in tqdm(ckpts):
    for dset_name in criterion_encode.keys():
      train_file_path = os.path.join(src.vars.PREPROCESS_DATA_PATH, f'{dset_name}_train.txt')
      eval_file_path = os.path.join(src.vars.PREPROCESS_DATA_PATH, f'{dset_name}_{args.split}.txt')
      ...
      os.system(f'perl {score_script_path} {train_file_path} {eval_file_path} {infer_file_path} > {out_f1_file_path}')
      ...
      recall = re.match(r'={3}\s+TOTAL\s+TRUE\s+WORDS\s+RECALL:\s+0\.(\d+)', lines[0]).group(1)
      precision = re.match(r'={3}\s+TOTAL\s+TRUE\s+WORDS\s+PRECISION:\s+0\.(\d+)', lines[1]).group(1)
      f1 = re.match(r'={3}\s+F\s+MEASURE:\s+0\.(\d+)', lines[2]).group(1)
      oov_recall = re.match(r'={3}\s+OOV\s+Recall\s+Rate:\s+0\.(\d+)', lines[4]).group(1)
      iv_recall = re.match(r'={3}\s+IV\s+Recall\s+Rate:\s+0\.(\d+)', lines[5]).group(1)

      if args.use_unc:
        writer.add_scalar(f'{dset_name}/{args.split}/unc/recall', recall)
        writer.add_scalar(f'{dset_name}/{args.split}/unc/precision', precision)
        writer.add_scalar(f'{dset_name}/{args.split}/unc/f1', f1)
        writer.add_scalar(f'{dset_name}/{args.split}/unc/oov_recall', oov_recall)
        writer.add_scalar(f'{dset_name}/{args.split}/unc/iv_recall', iv_recall)
      else:
        writer.add_scalar(f'{dset_name}/{args.split}/{dset_name}/recall', recall)
        writer.add_scalar(f'{dset_name}/{args.split}/{dset_name}/precision', precision)
        writer.add_scalar(f'{dset_name}/{args.split}/{dset_name}/f1', f1)
        writer.add_scalar(f'{dset_name}/{args.split}/{dset_name}/oov_recall', oov_recall)
        writer.add_scalar(f'{dset_name}/{args.split}/{dset_name}/iv_recall', iv_recall)
