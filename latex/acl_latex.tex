...
As point out first by \cite{chen-etal-2017-adversarial} and followed by many researches, the degeneration is mainly due to cross-dataset label inconsistency caused by different lingustic perspective.
...
\begin{table}[t!]
  ...
  \begin{tabular}{lll}
    ...
    \hline
  \end{tabular}
  \caption{Actual samples from different datasets \cite{emerson-2005-second} demonstrate labeling inconsistency.
...
Experiments show that MCCWS indeed perform better then CWS model without multi-criteria learning, and can achieve comparable performance compare to SOTA single model per dataset \cite{ke2020unified,ke-etal-2021-pre,tong-etal-2022-word}.
...
% Our experiments showed that by fine-tuning BERT~\cite{devlin-etal-2019-bert} on single CWS dataset, \(97\%\) F1-score can be easily achieved.
...
% Our work in some way is similar to \cite{ke-etal-2021-pre}.
...
\begin{table}[t]
  \caption{Number of words in training, development and test sets for all CWS datasets.}
...
\begin{table*}[t]
  ...
  \resizebox{2.1\columnwidth}{!}{
    \begin{tabular}{lccccccccc}
      \hline
      \textbf{MCCWS Models}                                    & \textbf{AS}    & \textbf{CITYU} & \textbf{CNC}  & \textbf{MSRA} & \textbf{PKU}  & \textbf{Avg-4} & \textbf{Avg-5} \\
      \hline
      Model-I+ADV~\cite{chen-etal-2017-adversarial}            & 94.64          & 95.55          & -             & 96.04         & 94.32         & 95.14          & -              \\
      BiLSTM+CRF~\cite{He-2019-effective}                      & 95.4           & 96.2           & -             & 97.4          & 95.9          & 96.23          & -              \\
      RoBERTa+fine-tune~\cite{huang-etal-2020-joint-multiple}  & -              & -              & 97.19         & 98.29         & 96.85         & -              & -              \\
      BERT+CRF~\cite{huang-etal-2020-towards}                  & 97.0           & 97.8           & \textbf{97.3} & \textbf{98.5} & \textbf{97.3} & \textbf{97.65} & \textbf{97.58} \\
      Unified Transformer Encoder~\cite{qiu-etal-2020-concise} & 96.44          & 96.91          & -             & 98.05         & 96.41         & 96.95          & -              \\
...
Our MCCWS model achieved \(97.3\%\) on average F1-score over four (five) CWS datasets, which is only \(0.35\%\) (\(0.28\%\)) less comparing to SOTA MCCWS model.
We achieve almost SOTA performance on CNC with only \(0.01\%\) less.
This is a suprise to us since we didn't use any complex architecture (like CRF) and handcrafted features (like bigram).
...
Table~\ref{tab:f1} shows that we can achieve \(96.21\%\) (\(96.35\%\)) on averge F1-score over four (five) CWS datasets, which is only \(1.44\) (\(1.23\%\)) less compare to SOTA MCCWS model.
This suggests our MCCWS model is indeed a practical tool.

\subsection{Ablation Study}

\begin{table}[t]
  \caption{Criterion prediction accuracy (in percentage) on test sets.}
  \label{tab:acc}
  \centering
  \resizebox{1\columnwidth}{!}{
    \begin{tabular}{lccccccc}
      ...
      \hline
      \UNC+\textbf{AS}    & 99.27       & 0.69           & 0.0          & 0.0           & 0.03         \\
      \UNC+\textbf{CITYU} & 5.69        & 94.17          & 0.0          & 0.0           & 0.13         \\
      \UNC+\textbf{CNC}   & 0.18        & 0.01           & 94.29        & 5.41          & 0.12         \\
      \UNC+\textbf{MSRA}  & 0.4         & 0.05           & 27.55        & 63.09         & 8.91         \\
      \UNC+\textbf{PKU}   & 0.05        & 0.13           & 8.78         & 21.29         & 69.73        \\
      \hline
    \end{tabular}
  }
\end{table}
  ...
  \resizebox{1\columnwidth}{!}{
    \begin{tabular}{lccccccc}
      \hline
                     & \textbf{AS}    & \textbf{CITYU} & \textbf{CNC}   & \textbf{MSRA}  & \textbf{PKU}   \\
      \hline
      \textbf{AS}    & \textbf{96.60} & 97.17          & 96.95          & 95.08          & 95.22          \\
      \textbf{CITYU} & 95.54          & \textbf{97.85} & 96.07          & 96.60          & 94.95          \\
      \textbf{CNC}   & 96.51          & 97.67          & \textbf{97.29} & 90.85          & 95.25          \\
      \textbf{MSRA}  & 95.92          & 97.15          & 94.06          & \textbf{97.92} & 94.42          \\
      \textbf{PKU}   & 95.63          & 97.07          & 92.78          & 87.80          & \textbf{96.84} \\
      \hline
    \end{tabular}
  }
\end{table}
...
% Notes
% (1) point out the similarity of between us and \cite{ke-etal-2021-pre}.
...
%     \hline
%     \cite{Gusfield:97}       & \verb|\cite|           & \verb|\cite|                   \\
...
% You can use the command \verb|\cite| (cite in parentheses) to get ``(author, year)'' citations \cite{Gusfield:97}.
